        ...
        {variants.find(x => x.attributes.is_discount_variant === true) ? 
          <div className='px-2 py-1 mb-2'>
            <div className='flex flex-col gap-1'>
              ...
              <p className='font-bold text-center text-red-500'>{getCurrency(Math.min(...variants.map(val => parseInt(val.attributes.variant_price) - (parseInt(val.attributes.variant_price) * val.attributes.variant_discount / 100 ))))}</p>
